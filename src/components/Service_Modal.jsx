...
// Lista de servicios con descripciones más detalladas
const services = [
    { 
        img: Service01.src, 
...
const ServiceModal = ({ onClose }) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center  bg-opacity-50 z-50">
            <motion.div 
                className="bg-sky-100 p-6 rounded-lg shadow-lg max-w-5xl w-full"
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{ duration: 0.4 }}
            >
                <h2 className="text-2xl font-bold text-sky-900 text-center">NUESTROS SERVICIOS</h2>
                ...
                <Slider {...settings} className="mt-6">
                    {services.map((service, index) => (
                        ...
                        >
                            <div className="bg-gray-100 p-4 rounded-lg shadow-lg text-center h-[390px] w-[350px]">
                                <motion.img 
                                    src={service.img} 
                                    alt={service.title} 
                ...
                <motion.button 
                    className="mt-12 bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700 block mx-auto"
                    onClick={onClose} 
